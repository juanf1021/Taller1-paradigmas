...
def programa(materias_inscri):
  pedir_datos()
  salir = True
  escrito = True
  cancelar = True
  numero_carrera = pedir_carrera()
  ...
  while salir:
    ...
    if inscribir == 1:
      inscribir_materias(escrito, numero_carrera, materia_seleccionada)
    else:
      cancelar_materias(cancelar, materia_seleccionada)
    salir = int(
      input(
        "\nDigite '1' para volver al menu principal\n\n Digite '0' para salir\n\n : "
      ))
  materias_inscritas(materias_inscri)


def inscribir_materias(escrito, numero_carrera, materia_seleccionada):
  while escrito:
    if numero_carrera == 1:
    ...
    if materia_seleccionada:
      ...
      materias_inscri.append(materia_seleccionada[materia_ins - 1])
      materia_seleccionada.remove(materia_seleccionada[materia_ins - 1])
      print(f"\nTienes {len(materias_inscri)}  materias inscritas")
    else:
      print("No hay materias por escribir ")
    escrito = int(
      input(
...
def cancelar_materias(cancelar, materia_seleccionada):
  while cancelar:
    if len(materias_inscri) == 0:
      print("No tienes materia disponibles para cancelar")
      cancelar = False
    else:
      for materia in materias_inscri:
        print((materias_inscri.index(materia)) + 1, ":", materia)
      materia_ins = int(input("Materia a Cancelar: "))
      materia_seleccionada.append(materias_inscri[materia_ins - 1])
      materias_inscri.remove(materias_inscri[materia_ins - 1])
    cancelar = int(
      input(
...
def materias_inscritas(materias_inscri):
  if not materias_inscri:
    print("No tienes materias inscritas")
  else:
...
def pedir_carrera(numero_carrera=0):
